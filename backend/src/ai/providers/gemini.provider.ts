...
export class GeminiProvider implements IAiProvider, OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      // Sử dụng Dynamic Import cho ESM module trong môi trường CommonJS
      const { GoogleGenAI: GoogleGenAIConstructor } =
        (await import('@google/genai')) as unknown as {
          GoogleGenAI: new (config: { apiKey: string }) => GoogleGenAI;
        };
      this.client = new GoogleGenAIConstructor({
        apiKey: apiKey || '',
      });
    } catch (error) {
