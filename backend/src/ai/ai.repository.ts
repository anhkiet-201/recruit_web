...
export class AiJobRepository {
  ...
  async updateUserEmbedding(userId: string, vectorString: string) {
    return this.prisma
      .$executeRaw`UPDATE "User" SET "embedding" = ${vectorString}::vector WHERE "id" = ${userId}`;
  }
  ...
  async recommendJobsForUser(userId: string, limit: number = 5) {
    return this.prisma
      .$queryRaw`SELECT j.id, j.title, 1 - (j."embedding" <=> u."embedding") as score FROM "Job" j, "User" u WHERE u.id = ${userId} AND j."isActive" = true AND j."embedding" IS NOT NULL ORDER BY score DESC LIMIT ${limit}`;
  }
}
