...
export default function Dropdown({
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            const isClickInTrigger = containerRef.current && containerRef.current.contains(target as Node);
            ...
            <div className="relative">
                ...
                >
                    <div className="flex items-center gap-2.5 truncate mr-2">
                        {Icon && <Icon size={iconSize} className={isOpen ? "text-blue-600" : "text-gray-400"} />}
                        <span className={`truncate ${selectedOption ? "text-gray-900" : "text-gray-400 font-medium"}`}>
                            {selectedOption ? selectedOption.label : placeholder}
                        </span>
                    </div>
                    <ChevronDown size={iconSize} className={`text-gray-400 transition-transform duration-300 ${isOpen ? "rotate-180 text-blue-600" : ""}`} />
                </button>
                    ...
                    <AnimatePresence>
                        {isOpen && coords.width > 0 && (
                            <motion.div
                                ...
                                className="fixed z-[100000] bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-100 overflow-hidden dropdown-portal-menu"
                            >
                                            ...
                                            <li key={option.value}>
                                                ...
                                                >
                                                    <div className="flex items-center gap-2.5">
                                                        {OptionIcon && <OptionIcon size={iconSize} className={isSelected ? "text-white/80" : "text-gray-400 group-hover:text-blue-500"} />}
                                                        <span>{option.label}</span>
                                                    </div>
                                                    {isSelected && <Check size={iconSize} className="text-white" />}
                                                </button>
                                            </li>
                                        );
                                    })}
