...
const JobCard = memo(function JobCard({
  ...
  const getJobTypeLabel = (type?: string) => {
    switch (type) {
      case "unskilled":
        return {
          label: tDetail("types.unskilled"),
          icon: Zap,
          color: "text-emerald-600 bg-emerald-50/80 border-emerald-100/50",
        };
      case "professional":
        return {
          label: tDetail("types.professional"),
          icon: Award,
          color: "text-blue-600 bg-blue-50/80 border-blue-100/50",
        };
      case "skilled":
        return {
          label: tDetail("types.skilled"),
          icon: GraduationCap,
          color: "text-fuchsia-600 bg-fuchsia-50/80 border-fuchsia-100/50",
        };
      default:
        return {
          label: tDetail("types.default"),
          icon: Zap,
          color: "text-gray-600 bg-gray-50/80 border-gray-100/50",
        };
    }
  };
      ...
      <div className="h-full bg-white rounded-[2.5rem] p-3.5 shadow-xl shadow-gray-200/40 border border-gray-100/60 hover:shadow-2xl hover:shadow-blue-200/50 hover:-translate-y-2.5 transition-all duration-500 flex flex-col relative overflow-hidden">
        ...
        <div className="relative aspect-16/10 w-full rounded-[2.2rem] overflow-hidden shadow-sm bg-gray-50 z-10">
          ...
          <div className="absolute bottom-4 left-4 z-20">
            <div
              className={`${typeInfo.color} backdrop-blur-md px-3.5 py-1.5 rounded-2xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 shadow-sm border w-fit`}
            >
              <TypeIcon size={12} />
              {typeInfo.label}
            </div>
        ...
        <div className="px-5 pt-7 pb-4 flex flex-col flex-1 z-10">
          ...
          <div className="grid grid-cols-2 gap-y-4 gap-x-2 mb-6">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[11px] font-bold text-gray-500 group-hover:text-gray-700 uppercase tracking-tight transition-colors">
                {job.experienceYears
                  ? t("yearsExp", { count: job.experienceYears })
                  : t("noExp")}
              </span>
            </div>
            <div className="flex items-center gap-2.5">
          ...
          {job.jobTags && job.jobTags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-8">
              {job.jobTags.slice(0, 3).map((jt) => (
                <span
                  key={jt.tagId}
                  className="px-2.5 py-1 bg-gray-100/80 text-[10px] font-bold text-gray-400 group-hover:text-gray-500 group-hover:bg-white border border-transparent group-hover:border-gray-100 rounded-lg tracking-wide transition-all shadow-sm"
                >
                  #{jt.tag.name}
                </span>
              ))}
              {job.jobTags.length > 3 && (
                <span className="text-[10px] font-black text-gray-300 px-1 py-1">
                  ...
                </span>
              )}
            </div>
          ...
          <div className="mt-auto flex items-center justify-between bg-gray-50/50 rounded-3xl p-2.5 pl-5 border border-gray-100/50 group-hover:bg-blue-50/50 transition-colors duration-500">
            <div className="flex items-center gap-2 text-[10px] font-black text-gray-400 group-hover:text-blue-600/70 uppercase tracking-widest transition-colors">
              ...
              <span>
                {job.deadline
                  ? t("deadline", { date: formatDate(job.deadline) })
                  : t("noDeadline")}
              </span>
            </div>
