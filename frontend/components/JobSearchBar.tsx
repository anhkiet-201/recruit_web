...
export default function JobSearchBar({
  ...
  const [prevInitialAiMode, setPrevInitialAiMode] = useState(initialAiMode);
  const [prevDefaultTitle, setPrevDefaultTitle] = useState(defaultTitle);

  if (initialAiMode !== prevInitialAiMode) {
    setPrevInitialAiMode(initialAiMode);
    setIsAiMode(initialAiMode);
  }

  if (defaultTitle !== prevDefaultTitle) {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full space-y-6">
      ...
      <form
        ...
        className={`bg-white/80 backdrop-blur-xl p-2.5 rounded-[2.5rem] shadow-2xl border flex flex-col md:flex-row gap-2 relative z-40 transition-all duration-500 ease-out ${
          isAiMode
            ? "border-blue-200 ring-8 ring-blue-500/5 shadow-blue-100"
      ...
      >
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 font-black px-12 py-4 rounded-4xl transition-all active:scale-95 shadow-xl whitespace-nowrap ${
            isAiMode
