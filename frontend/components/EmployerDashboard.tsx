...
export default function EmployerDashboard() {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      showDialog("Thành công", "Cập nhật thông tin thành công!", "success");
    } catch (error) {
      console.error("Failed to update profile", error);
      showDialog("Lỗi", "Không thể cập nhật thông tin.", "error");
    } finally {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0] && profile?.id) {
      ...
      try {
        ...
        await refreshProfile();
        showDialog("Thành công", "Cảnh đại diện đã được cập nhật.", "success");
      } catch (error) {
        console.error("Failed to upload avatar", error);
        showDialog("Lỗi", "Không thể tải ảnh lên.", "error");
      } finally {
  ...
  const handleApplicationStatus = async (
    ...
    try {
      ...
      showDialog("Lỗi", "Failed to update status", "error");
    }
  };
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          },
          {
            ...
            value: jobs.filter((j) => j.status === JobStatus.ACTIVE).length,
            icon: CheckCircle,
            color: "emerald",
          },
          {
          ...
          <Card
            key={i}
            className={`relative overflow-hidden border-none shadow-xl hover:shadow-2xl transition-all group`}
          >
            <div
              className={`absolute top-0 right-0 w-32 h-32 -mr-8 -mt-8 bg-${stat.color}-500/5 rounded-full blur-3xl group-hover:bg-${stat.color}-500/10 transition-colors`}
            ></div>
            <div className="flex items-center gap-5 relative z-10">
              <div
                className={`w-14 h-14 bg-${stat.color}-50 flex items-center justify-center rounded-2xl text-${stat.color}-600 border border-${stat.color}-100 shadow-sm`}
              >
                <stat.icon size={28} />
              </div>
              <div>
      ...
      <Card
        noPadding
        className="border-none shadow-2xl overflow-hidden bg-white/80 backdrop-blur-xl"
      >
        {/* Custom Tabs */}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-500 text-sm font-medium">
                        Thông tin này sẽ hiển thị trên các tin tuyển dụng của
                        bạn.
                      </p>
                    </div>
                    <Button
                ...
                <Input
                  ...
                  placeholder={
                    activeTab === "jobs"
                      ...
                      : "Tìm tên ứng viên hoặc vị trí ứng tuyển..."
                  }
                  value={searchTerm}
                        ...
                        >
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 rounded-2xl overflow-hidden border border-gray-100 shrink-0 shadow-sm transition-transform group-hover:scale-105 relative">
                                <Image
                                  src={
                                    job.imageUrl ||
                                    "https://placehold.co/100?text=Job"
                                  }
                                  alt=""
                                  fill
                                  className="object-cover"
                              ...
                              <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  {job.jobType && (
                                    <span className="text-[10px] py-0.5 px-2 bg-gray-100 text-gray-500 rounded-md font-bold uppercase tracking-widest italic">
                                      {job.jobType}
                                    </span>
                                  )}
                                </div>
                          ...
                          <td className="px-6 py-6 text-center">
                            <div className="flex items-center justify-center gap-6">
                              ...
                              <div className="text-center border-l border-gray-100 pl-6">
                                ...
                                <p className="text-lg font-black text-gray-700 leading-none">
                                  {job.views || 0}
                                </p>
                              </div>
                            </div>
                          ...
                          <td className="px-6 py-6">
                            <Badge
                              ...
                              isDot
                              className="px-3 py-1 text-[9px] shadow-sm"
                            >
                              {job.status}
                            </Badge>
                          </td>
                    ...
                    {filteredApps.length === 0 ? (
                      ...
                      filteredApps.map((app) => (
                        <ApplicationTableRow
                          ...
                          onStatusChange={(val) =>
                            handleApplicationStatus(app.id, val)
                          }
                        />
                      ))
                    )}
