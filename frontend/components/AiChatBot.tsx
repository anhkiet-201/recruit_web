...
export default function AiChatBot() {
  ...
  const SUGGESTIONS = [
    {
      icon: Briefcase,
      text: t("suggestions.findJob"),
      query: t("suggestions.findJob"),
    },
    {
      icon: FileText,
      text: t("suggestions.cvTips"),
      query: t("suggestions.cvTips"),
    },
    {
      icon: DollarSign,
      text: t("suggestions.salary"),
      query: t("suggestions.salary"),
    },
    {
      icon: Lightbulb,
      text: t("suggestions.interview"),
      query: t("suggestions.interview"),
    },
  ];
  ...
  useEffect(() => {
    ...
    if (savedSession) {
      try {
        const { messages: savedMessages, lastActive } =
  ...
  }, [INITIAL_MESSAGES]); // Run once on mount
  ...
  useEffect(() => {
    if (messages.length > 1) {
      localStorage.setItem(
        "chat_session",
        JSON.stringify({ messages, lastActive: Date.now() })
      );
    }
  }, [messages]);
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const historyContext: ChatHistoryItem[] = messages
      ...
      const { text, searchQuery, isSuggest, navigateUrl } = processBotResponse(
        result.response
      );
      ...
      setMessages((prev) => [
        ...prev,
        { role: "bot", text: t("error"), timestamp: Date.now() },
      ]);
  ...
  const handleNavigateFollowUp = async (
    jobId: string,
    currentHistory: ChatHistoryItem[]
  ) => {
    try {
   ...
   */
  const handleSearchFollowUp = async (
    query: string,
    currentHistory: ChatHistoryItem[]
  ) => {
    try {
      ...
      if (text) {
        setMessages((prev) => {
          ...
          return [
            ...updated,
            { role: "bot", text: text, timestamp: Date.now() },
          ];
        });
      }
    } catch (e) {
      ...
      setMessages((prev) =>
        prev.map((msg) => ({ ...msg, searchQuery: undefined }))
      );
    }
  };
    ...
    <>
        ...
        {!isOpen && (
          ...
          >
            ...
            <button
              ...
              className="w-16 h-16 bg-linear-to-br from-blue-600 to-indigo-700 rounded-full flex items-center justify-center text-white shadow-2xl hover:scale-110 active:scale-95 transition-all border border-white/20"
            >
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className="shrink-0 px-6 py-4 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-center sm:justify-between z-10">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-linear-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg text-white">
                  <Sparkles size={20} />
                </div>
                <div>
            ...
            <div
              ...
              className="flex-1 overflow-y-auto p-4 space-y-6 scroll-smooth bg-linear-to-b from-white/50 to-blue-50/30"
            >
