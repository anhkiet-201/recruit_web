...
export const JobService = {
  ...
  approveJob: async (id: string, status: JobStatus): Promise<Job> => {
    return api.patch(`/jobs/${id}/approve`, { status });
  },

  importJobs: async (
    file: File
  ): Promise<{ count: number; errors: { row: number; error: string }[] }> => {
  ...
  deleteJobImage: async (_imageUrl: string): Promise<void> => {
