...
export default function AdminUsersPage() {
  ...
  const handleRoleChange = (userId: string, newRole: string) => {
    const role = newRole as "candidate" | "admin" | "employer";
    setUsers((prev) => prev.map((u) => (u.id === userId ? { ...u, role } : u)));
    if (selectedUserForView?.id === userId)
      setSelectedUserForView({ ...selectedUserForView, role });
  };
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <Button icon={UserPlus} onClick={() => setShowCreateDialog(true)}>
          Add New User
        </Button>
      </div>
      ...
      <Card
        noPadding
        className="border-none shadow-2xl p-4 bg-gray-50/30 flex items-center"
      >
        ...
        {filteredUsers.length > 0 ? (
          filteredUsers.map((user) => (
            <Card
              key={user.id}
              noPadding
              className="group hover:-translate-y-1.5 transition-all duration-500 cursor-pointer overflow-visible"
            ...
            >
              <div className="p-8 pb-6">
                ...
                <div className="mt-6 pt-6 border-t border-gray-50 space-y-3">
                  ...
                  {user.phone && (
                    <div className="flex items-center text-sm text-gray-500 font-medium gap-3">
                      <Phone size={16} className="text-gray-300" /> {user.phone}
                    </div>
                  )}
                  {user.address && (
                    <div className="flex items-center text-sm text-gray-500 font-medium gap-3">
          ...
          <div className="col-span-full py-20 text-center bg-white rounded-4xl border-2 border-dashed border-gray-100">
