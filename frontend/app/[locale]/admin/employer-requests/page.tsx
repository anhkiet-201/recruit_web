...
import { Check, X, Mail, Phone, MapPin, Calendar, User as UserIcon, RotateCcw } from "lucide-react";
    ...
    return (
        <div className="p-8 space-y-8">
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tight">Employer Requests</h1>
                    <p className="text-gray-500 font-medium mt-1">Manage users who want to become employers.</p>
                </div>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-gray-50/50 border-b border-gray-100">
                            <th className="px-6 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest">User</th>
                            <th className="px-6 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest">Contact</th>
                            <th className="px-6 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest">Request Date</th>
                        ...
                        {loading ? (
                            ...
                            requests.map((request) => (
                                <tr key={request.id} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center font-black border border-blue-100 overflow-hidden">
                                                {request.user?.avatarUrl ? (
                                                    ...
                                                    request.user?.name?.charAt(0) || <UserIcon size={18} />
                                                )}
                                            </div>
                                            <div>
                                                <p className="font-black text-gray-900 leading-tight">{request.user?.name || "Unknown"}</p>
                                                <p className="text-xs text-gray-400 font-bold mt-0.5">{request.user?.email}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                    ...
                                    <td className="px-6 py-5">
                                        <Badge
                                            variant={getRequestStatusColor(request.status)}
                                        >
                                    ...
                                    <td className="px-6 py-5 text-right">
                                        ...
                                        {request.status === 'approved' && (
                                            <div className="flex justify-end">
                                                ...
                                                >
                                                    <RotateCcw size={16} />
                                                    <span className="text-[10px] font-black uppercase tracking-widest">Thu hồi quyền</span>
                                                </Button>
                                            </div>
                                        )}
                                        {request.status === 'rejected' && (
                                            <div className="flex justify-end">
                                                ...
                                                >
                                                    <RotateCcw size={14} />
                                                    <span className="text-[10px] font-black uppercase tracking-widest">Xem xét lại</span>
                                                </Button>
                                            </div>
                                        )}
                                    </td>
