  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const tagsToAdd = selectedTags.filter(
        (tId) => !initialTags.includes(tId)
      );
      const tagsToRemove = initialTags.filter(
        (tId) => !selectedTags.includes(tId)
      );
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-4">
      <div className="mb-10 flex items-center gap-4">
        <Link
          href="/admin/jobs"
          className="p-2 text-gray-400 hover:text-blue-600 bg-white rounded-xl shadow-md border border-gray-100 transition-all"
        >
          <ArrowLeft size={20} />
        </Link>
        <h1 className="font-black text-gray-900 tracking-tight uppercase text-xs opacity-40">
          Edit Mode
        </h1>
      </div>
      <JobForm
