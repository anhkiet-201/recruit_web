...
export default function ManageJobsPage() {
  ...
  const locationOptions = [
    ...
    ...Array.from(
      new Set(jobs.map((j) => j.location).filter((t): t is string => !!t))
  ...
  const handleApprove = async (id: string, status: JobStatus) => {
    try {
      await JobService.approveJob(id, status);
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSelectedJobIds(
      e.target.checked ? filteredJobs.map((job) => job.id) : []
    );
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (applicationCount > 0) {
      const ok = await confirm({
        ...
        message:
          "This job has generated applications and cannot be deleted. Would you like to mark it as expired instead to stop receiving new applications?",
      ...
      if (ok) {
        try {
          ...
          setJobs((prev) =>
            prev.map((j) =>
              j.id === id ? { ...j, status: JobStatus.EXPIRED } : j
            )
          );
        } catch (e) {
          console.error(e);
          await confirm({
            title: "Error",
            message: "Failed to expire job. Please try again.",
            confirmText: "OK",
            isDanger: true,
          });
        }
  ...
  const handleBulkStatus = async (status: JobStatus) => {
    try {
      setJobs((prev) =>
        prev.map((job) =>
          selectedJobIds.includes(job.id) ? { ...job, status: status } : job
        )
      );
      await Promise.all(
        selectedJobIds.map((id) => JobService.updateJob(id, { status: status }))
      );
    } catch (e) {
      console.error(e);
      loadJobs();
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">
            Create and manage your recruitment postings
          </p>
        </div>
              ...
              {filteredJobs.map((job) => (
                <tr
                  ...
                  className={`hover:bg-blue-50/30 transition-colors ${
                    selectedJobIds.includes(job.id) ? "bg-blue-50/50" : ""
                  }`}
                >
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-4">
                      <div className="h-12 w-12 rounded-2xl overflow-hidden border border-gray-100 shadow-sm relative shrink-0">
                        <Image
                          src={
                            job.imageUrl || "https://placehold.co/100?text=Job"
                          }
                          alt=""
                          width={48}
                          height={48}
                      ...
                      <div>
                        <p className="font-black text-gray-900 line-clamp-1">
                          {job.title}
                        </p>
                        <p className="text-xs text-gray-400 font-bold flex items-center gap-1 mt-0.5 uppercase">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      <button
                        ...
                        onClick={() =>
                          setAuthorSearchTerm(job.author?.name || "Admin")
                        }
                      >
                        <p className="text-sm font-bold text-gray-700">
                          {job.author?.name || "Admin"}
                        </p>
                      </button>
                      <button
                        className="text-left hover:text-blue-600 transition-colors"
                        onClick={() =>
                          setAuthorSearchTerm(job.author?.email || "")
                        }
                      >
                        <p className="text-[10px] font-bold text-gray-400 uppercase">
                          {job.author?.email || "System"}
                        </p>
                      </button>
                    </div>
                  </td>
                    ...
                    {(() => {
                      return (
                        <button onClick={() => handleToggleStatus(job)}>
                          <Badge variant={getJobStatusColor(job.status)} isDot>
                            {job.status}
                          </Badge>
                        </button>
                      );
                      ...
                      {job.status === JobStatus.REVIEWING && (
                        <>
                          <Button
                            ...
                            onClick={() =>
                              handleApprove(job.id, JobStatus.ACTIVE)
                            }
                          />
                          <Button
                            variant="ghost"
                            ...
                            onClick={() =>
                              handleApprove(job.id, JobStatus.REJECTED)
                            }
                          />
                        </>
                      )}
        ...
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 bg-gray-900 text-white rounded-3xl shadow-2xl p-2.5 pl-6 flex items-center gap-6 animate-in slide-in-from-bottom-10 duration-500">
          ...
          <div className="flex gap-1">
            <Button
              ...
              onClick={() => handleBulkStatus(JobStatus.ACTIVE)}
            >
              Activate
            </Button>
            <Button
              ...
              onClick={() => handleBulkStatus(JobStatus.DRAFT)}
            >
              Draft
            </Button>
            <Button
          ...
          <Button
            ...
            className="bg-white/10 hover:bg-white/20 border-none p-2 rounded-2xl"
          >
            <X size={18} />
          </Button>
        </div>
