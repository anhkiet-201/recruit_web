...
export default function MyApplicationsPage() {
  ...
  }, [user, loading, router, loadApplications]);
  ...
  const filteredApplications = applications.filter((app) => {
    const title = (app.job as { title?: string })?.title?.toLowerCase() || "";
          ...
          filteredApplications.map((app) => {
            ...
            return (
              <Fragment key={app.id}>
                <Card
                  noPadding
                  className={`group transition-all duration-500 border-none shadow-[0_8px_30px_-5px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)] overflow-hidden cursor-pointer ${
                    isExpanded
                ...
                >
                  <div className="p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div className="flex items-center gap-6">
                      <div
                        className={`h-16 w-16 rounded-2xl flex items-center justify-center transition-all duration-500 shadow-inner ${
                          isExpanded
                        ...
                        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              ...
                              <div className="flex items-center gap-3">
                                <div className="p-2 bg-white rounded-xl text-indigo-600 shadow-sm border border-slate-100">
                                  <Award size={16} />
                                </div>
                                <div className="flex flex-col">
                                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                  ...
                                  <span className="text-sm font-bold text-slate-700">
                                    {app.job?.experienceYears
                                      ? `${app.job.experienceYears} năm`
                                      : "Không yêu cầu"}
                                  </span>
                                </div>
                              </div>
                              <div className="flex items-center gap-3">
                                <div className="p-2 bg-white rounded-xl text-orange-600 shadow-sm border border-slate-100">
                                  <Calendar size={16} />
                                </div>
                                <div className="flex flex-col">
                                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          ...
                          <div className="flex flex-col justify-between">
                            ...
                            <div className="mt-8 p-4 bg-white/50 border border-slate-200/50 rounded-2xl">
                              <p className="text-xs text-slate-500 font-medium leading-relaxed">
                                <HelpCircle
                                  size={14}
                                  className="inline mr-1.5 text-blue-500 mb-0.5"
                                />
                                Đơn ứng tuyển của bạn hiện đang ở trạng thái{" "}
                                <span className="font-bold text-blue-600">
                                  {app.status}
                                </span>
                                . Nhà tuyển dụng sẽ xem xét hồ sơ của bạn và
                                liên hệ nếu phù hợp.
                              </p>
                            </div>
                          </div>
                  ...
                  <div className="absolute top-0 right-0 w-32 h-32 bg-linear-to-l from-blue-50/0 to-transparent rounded-bl-full -z-10 opacity-0 group-hover:from-blue-50/50 group-hover:opacity-100 transition-all duration-700"></div>
                </Card>
              </Fragment>
            );
          })
